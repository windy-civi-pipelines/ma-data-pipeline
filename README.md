...
2. **Name your new repository** using the convention: `Massachusetts Data Pipeline` (e.g., `il-data-pipeline`, `tx-data-pipeline`).
...
3. **Update the state abbreviation** in both workflow files:
   ...
   **In `.github/workflows/scrape-and-format-data.yml`:**
   ...
   ```yaml
   env:
     STATE_CODE: ma # CHANGE THIS to your state abbreviation
   ...
   **In `.github/workflows/extract-text.yml`:**
   ...
   - name: Extract text
     ...
     with:
       state: ma # CHANGE THIS to your state abbreviation
   ```
...
uses: windy-civi/toolkit/actions/scrape@main
with:
  state: ma # State abbreviation (required)
...
git clone https://github.com/YOUR-ORG/Massachusetts Data Pipeline
cd Massachusetts Data Pipeline
